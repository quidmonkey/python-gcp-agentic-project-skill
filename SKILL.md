...
- Code review: pre-push hook runs a two-pass agentic review (`scripts/code-review.sh`, configured via `.codereviewrc`, defaults to claude); blocks the push on REQUIRED findings (failing findings printed to the terminal), report in `working/code-review-report.md`, incremental per branch
