...
Findings come back as REQUIRED or SUGGESTED. Any REQUIRED finding fails the hook, blocks the push, prints the failing pass's findings to the terminal, and writes the full report to `working/code-review-report.md`. The project's CLAUDE.md tells the agent to read that report, fix REQUIRED findings at root cause, and push again.
